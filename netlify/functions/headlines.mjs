const J=(x,status=200)=>new Response(JSON.stringify(x),{status,headers:{'content-type':'application/json; charset=utf-8','cache-control':'no-store, no-cache, must-revalidate, max-age=0'}});
...
候選：${JSON.stringify(compact)}`;
 ...
 if(!r.ok){
   ...
   throw new Error(model+'｜HTTP '+r.status+'｜'+e.slice(0,260));
 }
...
async function geminiSelect(candidates){
 ...
 for(const model of models){
   try{
     const items=await callGeminiModel(model,candidates);
     if(items.length)return {items,model};
     errors.push(model+'｜no items');
   }catch(e){
...
export default async()=>{
 try{
  ...
  try{
   const aiResult=await geminiSelect(candidates),ai=aiResult.items,picked=[];for(const x of ai){const src=candidates[Number(x.id)];if(!src||!x.title_zh)continue;picked.push({title:String(x.title_zh).trim(),url:src.url,domain:src.domain,published:src.published,why:String(x.summary_zh||'').trim()});if(picked.length===5)break}
   return J({updatedAt:new Date().toISOString(),headlines:picked,ai:true,model:aiResult.model});
  }catch(e){
